<script lang="ts">
  ...
  export let count: number = 200;
  export let gravity: number = 0.7;
  export let friction: number = 0.8;
  export let wallBounce: number = 0.95;
  export let followCursor: boolean = true;
  export let colors: number[] = [0xff6b6b, 0x4ecdc4, 0xffe66d, 0xa8e6cf, 0xff8b94];
  export let ambientColor: number = 0xffffff;
  export let ambientIntensity: number = 1;
  export let lightIntensity: number = 200;
  export let minSize: number = 0.5;
  export let maxSize: number = 1;
  export let size0: number = 1;
  export let maxVelocity: number = 0.15;
  export let maxX: number = 5;
  export let maxY: number = 5;
  export let maxZ: number = 2;
  export let circularObstacles: Array<{x: number, y: number, radius: number}> = [];

  let canvas: HTMLCanvasElement;
  let ballpitInstance: CreateBallpitReturn | null = null;
  let canvasWidth = 0;
  let canvasHeight = 0;
  ...
  interface WConfig {
    ...
    circularObstacles?: Array<{x: number, y: number, radius: number}>;
  }
  ...
  class W {
    ...
    update(deltaInfo: { delta: number }) {
      ...
      for (let idx = startIdx; idx < config.count; idx++) {
        ...
        if (config.circularObstacles && config.circularObstacles.length > 0) {
          for (const obstacle of config.circularObstacles) {
            ...
            if (dist < minDist && dist > 0) {
              ...
              // Push ball outside the circular boundary
              ...
              vel.x = (vel.x - 2 * dotProduct * nx) * config.wallBounce;
              vel.y = (vel.y - 2 * dotProduct * ny) * config.wallBounce;
            }
          }
        }
        pos.toArray(positionData, base);
  ...
  function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
    const threeInstance = new X({
      canvas,
      size: config.width && config.height ? { width: config.width, height: config.height } : 'parent',
    ...
    // Call onAfterResize immediately to set initial boundaries
    threeInstance.onAfterResize(threeInstance.size);
    return {
      three: threeInstance,
      get spheres() {
        return spheres;
  ...
  onMount(() => {
    ...
    // Use async IIFE to wait for layout
    (async () => {
      // Wait for Svelte to complete rendering and layout
      await tick();

      // Force layout and read actual dimensions
      const rect = parent.getBoundingClientRect();
      const width = rect.width || parent.offsetWidth || window.innerWidth;
      const height = rect.height || parent.offsetHeight || window.innerHeight;

      console.log('Canvas dimensions:', { width, height });

      if (width === 0 || height === 0) {
        console.error('Canvas parent has zero dimensions!');
      }

      // Update reactive variables for tracking
      canvasWidth = width;
      canvasHeight = height;

      // Now initialize Three.js with correct explicit dimensions
      ballpitInstance = createBallpit(canvas, {
        count,
        gravity,
        friction,
        wallBounce,
    ...
    const handleResize = () => {
      if (parent && canvas) {
        const rect = parent.getBoundingClientRect();
        const newWidth = rect.width || parent.offsetWidth;
        const newHeight = rect.height || parent.offsetHeight;
  ...
  // Update obstacles when prop changes
  $: if (ballpitInstance?.spheres) {
    ballpitInstance.spheres.config.circularObstacles = circularObstacles;
  }

  // Subscribe to profile obstacle and convert to world coordinates
  $: if ($profileObstacle && ballpitInstance?.three && canvas) {
    const rect = canvas.getBoundingClientRect();

    // Convert screen coordinates to normalized device coordinates (-1 to 1)
    const ndcX = ($profileObstacle.screenX / rect.width) * 2 - 1;
    const ndcY = -($profileObstacle.screenY / rect.height) * 2 + 1;

    // Use raycaster to convert to world coordinates at z=0
    const raycaster = new Raycaster();
    const plane = new Plane(new Vector3(0, 0, 1), 0);
    const intersectionPoint = new Vector3();

    raycaster.setFromCamera(new Vector2(ndcX, ndcY), ballpitInstance.three.camera);
    raycaster.ray.intersectPlane(plane, intersectionPoint);

    // Calculate radius in world space
    // Use the height of the view to scale the radius proportionally
    const worldHeight = ballpitInstance.three.size.wHeight;
    const screenHeight = rect.height;
    const worldRadius = ($profileObstacle.screenRadius / screenHeight) * worldHeight;

    // Update the circular obstacles array
    circularObstacles = [{
      x: intersectionPoint.x,
      y: intersectionPoint.y,
      radius: worldRadius
    }];
  } else if (!$profileObstacle) {
    // Clear obstacles if profile is removed
    circularObstacles = [];
  }
</script>
